...
def possible_node(current_node, RPM1, RPM2, Graph_map, L = 28.7, R = 3.3):  
    ...
    for action in actions:
        ...
        velocities = []
        next_x = currn_node_x 
        next_y = curren_node_y 
        next_theta = curren_node_theta 
#--------------------------------------Calculating the next x, y and theta--------------------------------------#
        while t < 1:
            t += dt
            ...
            velocities.append(((linear_vel), (angular_vel))) # Storing the linear and angular velocities    
            next_x += del_x
            next_y += del_y
            next_theta += del_theta
            

        next_x = int(round(next_x)) # Rounding off the next x and y coordinates  
        next_y = int(round(next_y))  # Rounding off the next x and y coordinates
...
def A_star_Backtracting(parent, start_node, end_node, map_visualization, step_count):
    ...
    for i in range(len(parent_and_velocities) - 1):
        start_point, init_vel = ((parent_and_velocities[i][0][0]), parent_and_velocities[i][0][1]), parent_and_velocities[i][1] # Getting the start point and initial velocities.
        end_point = (parent_and_velocities[i + 1][0][0]), (parent_and_velocities[i + 1][0][1]) # Getting the end point.
        ...
        for lin_vel, ang_vel in init_vel:
            # Storing the linear and angular velocities in meters and radians.
